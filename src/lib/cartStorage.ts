...
/** Sepet satırı: quantity masaüstünde adet; web tabanlı (SAAS) ürünlerde kullanım süresi (yıl). */
...
export type CartLine = {
  productId: string
  quantity: number
  /** API cart-preview başarısız olsa bile sepet sayfasında göstermek için */
  snapshot?: CartSnapshot
}

/** cart-preview yanıtındaki id + matchKeys ile sepet satırlarının productId değerini kanonik UUID’ye hizalar. */
...
export type AddToCartOptions = {
  snapshot?: CartSnapshot
  /**
   * true: aynı ürün satırı varsa quantity bu çağrıdaki değere ayarlanır (detay sayfası web ürünü).
   * false/undefined: miktarlar toplanır (liste kartı, masaüstü detay).
   */
  replaceLine?: boolean
}
