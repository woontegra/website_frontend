...
type CheckoutLegalModalProps = {
  open: boolean
  title: string
  /** Son güncelleme satırı (ör. KVKK CMS etiketi) */
  updatedLabel?: string | null
  onClose: () => void
  children: ReactNode
  /** Zorunlu metinlerde isteğe bağlı ikinci kapatma */
    ...
    >
      ...
      >
        <header className="shrink-0 border-b border-slate-200 bg-slate-50/90 px-4 py-4 sm:px-6">
          ...
          {updatedLabel ? (
            <p className="mt-1 text-xs text-slate-500 sm:text-sm">Son güncelleme: {updatedLabel}</p>
          ) : null}
        </header>
