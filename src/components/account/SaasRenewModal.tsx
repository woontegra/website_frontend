...
import { customersApi, type CustomerSaasMembershipRow, type SaasRenewQuote } from '../../api/customers-api'
import {
...
export function SaasRenewModal({ membership, onClose, onOrderCreated }: Props) {
  ...
  useEffect(() => {
    ...
    void customersApi
      ...
      .catch((e) => {
        if (!cancelled) setQuoteError(e instanceof Error ? e.message : 'Fiyat alınamadı')
      })
      .finally(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError(null)
    try {
      const order = await customersApi.createSaasRenewOrder(membership.id, {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/50 p-4 sm:items-center" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Üyeliği Uzat</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-5 py-5">
          ...
          <div className="rounded-xl bg-slate-50 px-4 py-3">
            ...
            {quoteLoading ? (
              ...
              <p className="mt-1 text-2xl font-bold text-slate-900">
                {formatMoneyAmount(quote.total, quote.currency)}
              </p>
            ) : null}
          ...
          <div className="space-y-2 text-sm text-slate-700">
            ...
            <label className="flex gap-2">
              <input type="checkbox" checked={acceptDistance} onChange={(e) => setAcceptDistance(e.target.checked)} className="mt-0.5" />
            ...
            <label className="flex gap-2">
              <input type="checkbox" checked={acceptWaiver} onChange={(e) => setAcceptWaiver(e.target.checked)} className="mt-0.5" />
